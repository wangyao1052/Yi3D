...
for sel in ss:
    ...
    # 遍历草图图元
    # 将所有椭圆的圆心偏移(100.0, 100.0)
    # 将所有椭圆的长轴半径增大一倍,短轴半径维持不变
    for entityId in sketch:
        entityForRead = db.getElement(entityId)
        if not isinstance(entityForRead, wy3d.SketchEllipse):
            continue
        ellipse = trans.getElementForWrite(entityId)
        ellipse.setCenter(ellipse.getCenter() + wy3d.Vector2(100.0, 100.0))
        ellipse.setMajorAxis(ellipse.getMajorAxis() * 2)
        ellipse.setRadiusRatio(ellipse.getRadiusRatio() * 0.5, False)
