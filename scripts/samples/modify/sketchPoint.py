...
if ss.getCount() == 0:
    ...
    point = wy3d.SketchPoint.create(trans, wy3d.Vector2(0.0, 0.0))
...
for sel in ss:
    ...
    # 遍历草图图元
    # 将所有点的位置偏移(100.0, 100.0)
    for entityId in sketch:
        entityForRead = db.getElement(entityId)
        if not isinstance(entityForRead, wy3d.SketchPoint):
            continue
        point = trans.getElementForWrite(entityId)
        point.setPosition(point.getPosition() + wy3d.Vector2(100.0, 100.0))
